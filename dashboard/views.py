...
def get_departments(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            out_response["data"] = resp
            logger.info("Departments and objectives analytical output "
                        "response: %s" % str(out_response))
        except Exception as err:
            out_response["status"] = "ERROR"
            out_response["data"] = str(err)
            logger.error
            logger.error("Error while getting departments and objectives "
                         "analyticl data, Error: %s, Stack: %s" 
                         % (str(err), format_exc()))
        return HttpResponse(dumps(out_response), status=200)
    out_response["status"] = "ERROR"
    out_response["data"] = "Invalid HTTP method, use GET"
    return HttpResponse(dumps(out_response), status=200)
    ...
    if request.method == "GET":
        try:
            ...
            if department_name is None:
                logger.error("No department name is given.")
                out_response["status"] = "ERROR"
                out_response["data"] = "department name is not provided"
                return HttpResponse(dumps(out_response), status=200)
            ...
            logger.info("Output response for all the teams of a "
                        "department is: %s" % str(out_response))
        except Exception as err:
            out_response["status"] = "ERROR"
            out_response["data"] = str(err)
